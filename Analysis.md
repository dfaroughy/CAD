...
| Model | Code Pass | Median relative volume | Median hausdorff distance | Median $\xi_{L2}(r)$ 2-point correlator| combined p_value > 0 |
|---|---|---|---|---|---|
...
The precise meaning of thiese metrics are descibed below. 
...
3. **vol_rel**: difference between shape volumes, catches broad scale/topology errors.
4. **xi_l2**: Landy-Szalay 2-point correlation function (L2 norm). This is a common estimator in astrophysics and cosmology. It has the benefit of being invariant under rotations and reflexions. This may be a new metric in this context? 
5. **p_value**: combined significance test of all the metrics above. Itsaturates quickly, useful only if the model perfomred perfectly. Choice of these metrics are motivated by the correlation matrix:

![alt text](figs/metrics_results.png)
...
The benchmark exposes two problems requiring different interventions. 
 ...
 For the harder problem of achieving dimensional accuracy and complex geometry, supervised fine-tuning on (drawing image, ground-truth .py) pairs is the critical next step. The model needs to learn to read dimension annotations from the drawing, not just infer rough shape. the finetuning should be geared to solve that problem. This calls for chain-of-thought fine-tuning where the reasoning trace explicitly extracts each annotated dimension before writing a CadQuery line. Starting from Qwen2.5-VL-72B as the base (highest code reliability) with LoRA fine-tuning on this dataset is probably a good first single action.
